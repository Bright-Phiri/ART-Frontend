      ...
      <v-col cols="12">
            ...
            <v-card>
              ...
              <v-card-text>
                <v-form ref="consultationForm" v-on:submit.prevent="addLabOrder">
                  ...
                  <div class="d-flex justify-end">
                     <v-btn v-on:click="cancelLabOrder" dark class="secondary text-capitalize">Cancel</v-btn> <v-btn class="ml-2 text-capitalize" dark color="#35B4E4" type="submit">Add lab order</v-btn>
                  </div>
                </v-form>
              </v-card-text>
               <v-overlay absolute opacity="0" :value="overlay">
            ...
            <v-card>
              ...
              <v-card-text>
                <v-form ref="editPatientForm" v-on:submit.prevent="updatePatient">
                   ...
                   <div class="d-flex justify-end">
                      <v-btn v-on:click="cancelEdit" class="text-capitalize secondary">Cancel</v-btn> <v-btn type="submit" class="ml-2 text-capitalize" dark color="#35B4E4">Update patient</v-btn>
                   </div>
                </v-form>
              </v-card-text>
              <v-overlay absolute opacity="0" :value="overlay">
          ...
          <v-card tile>
            <v-card-title class="d-flex">
               <span>Patients</span>
               <v-spacer></v-spacer>
               <v-btn class="mr-2" outlined small dark color="#F11E14" v-on:click="exportToPdf"><v-icon left>mdi-file-export</v-icon>Export</v-btn>
               <v-btn class="text-capitalize" outlined small dark color="#35B4E4" v-on:click="addPatientDialog = !addPatientDialog"><v-icon left>mdi-account-plus</v-icon>New Patient</v-btn>
            </v-card-title>
            <v-card-text>
              <div class="col-lg-3 col-xl-3 col-xm-12 col-sm-5 col-md-3 pa-0"> <v-text-field outlined dense label="Search"  v-model="search"  append-icon="mdi-magnify"></v-text-field></div>
               <v-data-table dense :headers="headers"  class="elevation-3" :search="search" :items-per-page="7" :items="patients"  :loading="loading" loading-text="loading patients">
                   <template v-slot:[`item.action`]="{ item }">
                   <v-icon small class="mr-2" color="#35B4E4" v-on:click="showLabOrderDialog(item.id)">mdi-plus-box</v-icon>
                   <v-icon small class="mr-2" v-on:click="selectPatientRecord(item.id)" color="secondary">mdi-pencil</v-icon>
